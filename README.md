...
- Filter Service: fallback на координаты hints реализован — если LLM вернула сущность по hint без `start/end`, координаты подставляются из hint.
- DOCX: базовая версия извлекает текст из параграфов, таблицы могут игнорироваться (улучшение планируется).
...
- Улучшение извлечения текста из DOCX таблиц
